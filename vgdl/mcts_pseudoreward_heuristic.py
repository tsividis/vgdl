...
def planUntilSolved(rleCreateFunc, filename, defaultPolicyMaxSteps, partitionWeights, playback=False, maxEpisodes=2000):
	...
	i=0
	finalStates = [rle._game.getFullState()]
	mcts = Basic_MCTS(existing_rle=rle, game=game, level=level, partitionWeights=partitionWeights)
	subgoals = mcts.getSubgoals(subgoal_path_threshold=4)
	
	total_steps = 0
	solved = True
	for subgoal in subgoals:
		rle, steps = getToWaypoint(rle, subgoal, defaultPolicyMaxSteps, partitionWeights=partitionWeights)
		print steps, "steps"
		total_steps += steps
