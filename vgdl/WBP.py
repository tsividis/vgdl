...
class WBP():
    def __init__(self, rle, gameFilename, theory=None, fakeInteractionRules = [], seen_limits=[], annealing=1, max_nodes=100000, shortHorizon=False,
        firstOrderHorizon=False, hyperparameters={}, extra_atom=False, planning_lesion=False, avatar_interactions_score=None, avatar_interactions_locations=None,
        projectile_interactions_locations=None):
        self.rle = rle
        ...
        self.avatar_interactions_score = avatar_interactions_score
        self.avatar_interactions_locations = avatar_interactions_locations
        self.projectile_interactions_locations = projectile_interactions_locations
...
## multiple samples??
    ...
    def rollout(self, Vrle):
        ...
        while not successfulRollout:
            ...
            if not self.WBP.planning_lesion:
                prevHeuristicVal = self.heuristics(vrle, **self.WBP.rolloutHyperparameters)
            else:
                prevHeuristicVal = self.lesioned_heuristics(vrle)
            rolloutArray = []
            i=0
            ...
            while i<self.rolloutDepth and not terminal:
                ...
                if not self.WBP.planning_lesion:
                    ...
                    currHeuristicVal = self.lesioned_heuristics(vrle)
    ...
    def getColorFromID(self, rle, id):
        color = None
        try:
            color = rle._game.all_objects[id]['sprite'].color
        except KeyError:
            if id in [obj.ID for obj in rle._game.kill_list]:
                color = [obj.color for obj in rle._game.kill_list
                    if obj.ID==id][0]

            elif id in rle._game.getObjects().keys():
                color = rle._game.getObjects()[id]['sprite'].color
        return color
    ...
    def lesioned_heuristics(self, rle=None, effectList=None, H=1000000.):
        if rle==None:
            rle = self.rle
        ...
        heuristicVal = 0
        # Winning Maximization as in Guerrero-Romero et al. 2017
        ...
        # Exploration Maximization as in Guerrero-Romero et al. 2017
        pos = self.get_position(rle)
        if pos and self.WBP.visited_positions[pos[0]/self.WBP.pixel_size, pos[1]/self.WBP.pixel_size] == 0:
            heuristicVal += H/100.
            emh_flag = False

        elif self.parent:
            parent_pos = self.parent.get_position(rle)
            if parent_pos == pos:
                heuristicVal -= H/200.
                emh_flag = False
        ...
        if effectList:
            for e in effectList:
                # Knowledge Discovery and Knowledge Estimation as in
                # Guerrero-Romero et al. 2017

                ended, win = rle._isDone()


                color1, color2 = self.getColorFromID(rle, e[1]), self.getColorFromID(rle, e[2])
                if color1 and color2:
                    color1, color2 = colorDict[str(color1)], colorDict[str(color2)]

                    avatar_color = 'DARKBLUE'

                    if avatar_color in [color1, color2]:
                        partner = color1 if avatar_color==color2 else color2

                        if tuple(pos) not in self.WBP.avatar_interactions_locations[partner]:
                            heuristicVal += H/400
                            kdh_flag = False
                        if self.WBP.avatar_interactions_score[partner] != 0:
                            heuristicVal += self.WBP.avatar_interactions_score[partner] * H/100
                        # self.avatar_interactions_locations[partner].append()
                    try:
                        projectile_name = rle._game.getAvatars()[0].stype
                        projectile_color = [o for o in rle._game.getObjects().values() if projectile_name==o['sprite'].name][0]['sprite'].color
                        projectile_color = colorDict[str(projectile_color)]

                        if projectile_color in [color1, color2]:
                            partner = color1 if projectile_color==color2 else color2

                            if tuple(pos) not in self.WBP.projectile_interactions_locations[partner]:
                                heuristicVal += H/400
                                kdh_flag = False
    ...
    def get_position(self, rle=None):
        try:
            (x, y) = np.array((rle._game.getAvatars()[0].rect.x,
                rle._game.getAvatars()[0].rect.y))
            return (x, y)
        except IndexError:
    ...
    def position_score(self, factor=1.):
        pos = self.get_position(self.rle)
        return factor * self.WBP.visited_positions[x, y] if pos else 0
        ...
        if self.parent and self.parent.rle is not None:
            ...
            try:
                ...
                if len(self.actionSeq)>0:
                    ...
                    if self.WBP.planning_lesion:
                        self.heuristicVal = self.parent.heuristicVal + self.lesioned_heuristics(vrle, res['effectList'])
    ...
    def eval(self):
        ...
        print("intrinsic_reward is {}".format(self.intrinsic_reward))
