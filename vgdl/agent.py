...
class Agent:
	...
	def expandTheories(self, theories, errorList, envRealPrev, envRealCurrent, prevAction):
		...
		if len(errorList)==1:
			...
			## Set max_num to None!
			scoresAndHypotheses = [(h[0],h[1]) for h in self.filterTheories(scoreAndTheoryTuples, percentile=0, max_num=5,
	...
	def executeStep(self, action, hypotheses, theoryRLEs, lastStep=False):
		...
		if newTheories:
			...
			if not lastStep:
				scoresAndHypotheses = [(h[0],h[1]) for h in self.filterTheories(scoreAndTheoryTuples, percentile=30, max_num=5,
					proportionOfSpriteTheories=None)]
			else:
