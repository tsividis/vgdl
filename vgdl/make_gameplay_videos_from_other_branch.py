...
from main_agent_for_videos import Agent
...
def initialize_agent(gameFileString, game_name, game_level, movie_name):
    start = time.time()
                        ...
                        for i, episodes in enumerate(levels_played):
                            for j, episode in enumerate(episodes):
                                ...
                                for frameIdx, frame in enumerate(episode):
                                    ...
                                    # browns = [tup for tup in frame['objects'] if tup[0]=='BROWN']
                                    # sortedbrowns = sorted(browns, key=lambda x:x[1][0])
                                    # newtups, newbrowns = [], []
                                    # for tup in sortedbrowns:
                                    #     newtup = tup
                                    #     if tup[1] in new_logs.keys():
                                    #         new_logs[tup[1]] += 1
                                    #     elif frameIdx>0 and tup[1][0]!=27:
                                    #         new_logs[tup[1]] = 1
                                    #         newtup = (tup[0], (tup[1][0]+1, tup[1][1]), tup[2])
                                    #     else:
                                    #         new_logs[tup[1]] = 0
                                    #     newtups.append(newtup)
                                    # sortedbrowns = sorted(newtups, key=lambda x:x[1][0])
                                    # for tup in sortedbrowns:
                                    #     if new_logs[tup[1]]%10 != 0:
                                    #         offset = new_logs[tup[1]]%10
                                    #         oldtup = tup
                                    #         tup = (tup[0], (tup[1][0] - (offset*.1),tup[1][1]), tup[2])
                                    #         print "updating {} to {}".format(oldtup, tup)
                                    #     else:
                                    #         print "not updating {}".format(tup)
                                    #     newbrowns.append(tup)


                                    ## good stuff
                                    # browns = [tup for tup in frame['objects'] if tup[0]=='BROWN']
                                    # updatedBrowns = [tup for tup in browns if tup[1][0]==27]
                                    
                                    # if frameIdx==0:
                                    #     previousBrowns = browns
                                    #     updatedBrowns = browns
                                    # else:
                                    #     for tup in previousBrowns:
                                    #         newtup = (tup[0], (round(tup[1][0]-.1,1), tup[1][1]), tup[2])
                                    #         updatedBrowns.append(newtup)

                                    ## you need at least this for printing to work.
                                    # previousBrowns = updatedBrowns

                                    # updatedBrowns = [tup for tup in frame['objects'] if tup[0]=='BROWN']
                                    # if frameIdx in [9,10,11]:
                                    # print frameIdx
                                    # print sorted([tup for tup in frame['objects'] if tup[0]=='BROWN'], key=lambda x:x[1][0])
                                    # print ""
                                    # print sorted(updatedBrowns, key=lambda x: x[1][0])
                                    # agent_sprites = [tup for tup in frame['objects'] if tup[0]=='DARKBLUE']
                                    # if j==5 and agent_sprites and agent_sprites[0][1][1]<5:
                                        # embed()
                                    # embed()

                                    # embed()
                                    # frame['objects'] = [tup for tup in frame['objects'] if tup[0]!='BROWN']
                                    # frame['objects'] = frame['objects']+updatedBrowns  
                                    # embed() 
                                    for tup in frame['objects']:
                                        # if tup[0]=='BROWN':
                                        #     if tup[1] in new_logs.keys():
                                        ...
                                        #     if new_logs[tup[1]]%10 != 0:
                                        #         offset = new_logs[tup[1]]
                                        #         tup = (tup[0], (tup[1][0] - (offset*.2),tup[1][1]), tup[2])

                                            # if tup[1][0]==26:
                                            #     if tup[1][1] not in new_logs.keys():
                                            #         new_logs[tup[1][1]]=0 ## just appeared
                                            #     else:
                                            #         new_logs[tup[1][1]]+=1
                                            #     if new_logs[tup[1][1]]>0: ## if it's not the first time this has appeared, add offset
                                            #         offset = new_logs[tup[1][1]]
                                            #         tup = (tup[0], (tup[1][0] - (offset*.2),tup[1][1]), tup[2])
                                            #     if new_logs[tup[1][1]]>5:
                                            #         new_logs.pop(tup[1][1])
...
def makeEMPAMovies(experimentsDict, dirName):
    ...
    for ID in experimentsDict:
            ...
            for gameNumber in sorted(experimentIndices.keys()):
                ...
                ## insert video stitching here. Delete to do original movie-making stuff
                videos_to_stitch = [d for d in os.listdir('tmp_videos/'+gameName) if 'DS' not in d]
                num_existing_videos = len([d for d in os.listdir('videos/'+gameName) if 'DS' not in d])

                f = open('videolist.txt', 'w+')
                for v in videos_to_stitch:
                    f.write("file 'tmp_videos/{}/{}'\n".format(gameName,v))
                f.close()

                com = "ffmpeg -f concat -safe 0 -i videolist.txt -c copy videos/{}/{}_{}.mp4".format(gameName,gameName,num_existing_videos)
                command = "{}".format(com)
                subprocess.call(command, shell=True)
                shutil.rmtree("tmp_videos/"+gameName)
                os.makedirs("tmp_videos/"+gameName)
...
makeEMPAMovies(loadedData, dirName)
