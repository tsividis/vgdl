...
class Agent(object):
    ...
    def planAsNeeded(self, force_replan=False):
        ...
        if self.re_plan==True:
            ...
            # t1 = time.time()
            p = WBP.WBP(self.theoryRLEs[0], self.gameFilename, theory=self.hypotheses[0], fakeInteractionRules = self.fakeInteractionRules,seen_limits = self.seen_limits, max_nodes=max(1, int(self.max_nodes * self.max_nodes_scale)),
    ...
    def step(self, action, env_results=None):
        ...
        if self.environment.getTime() < 5 or self.environment.getTime() % 1 == 0:
            ...
            self.logfMRIRegressor('spriteKL', 0)
