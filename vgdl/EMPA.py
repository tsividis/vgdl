...
class Agent:
    ...
    def step(self, env):
        ...
        if env.getTime()==1:
            ...
            self.all_objects= env.getObjects()
            annealing = 1
        
            ## Initialize memory of object positions
            self.memory.objectMemoryDict, self.memory.previousPositions = {}, {}
            for k, v in env._game.all_objects.iteritems():
                self.memory.objectMemoryDict[k] = (int(env._game.all_objects[k]['sprite'].rect.x), int(env._game.all_objects[k]['sprite'].rect.y))
                self.memory.previousPositions[k] = (int(env._game.all_objects[k]['sprite'].rect.x), int(env._game.all_objects[k]['sprite'].rect.y))
            ...
            if len(self.hypotheses) == 0:
                ...
                [t.updateTerminations(rle=env) for t in self.hypotheses]


            ## Do beginning-of-episode Avatar resource-management.
            resources = env.getAvatars()[0].resources
            for resource, val in resources.items():
                if resource not in self.seen_resources and val>0:
                    self.seen_resources.append(resource)
                    self.hypotheses[0].resource_limits[resource] = env.getResourceLimits()[resource]
                if resource not in self.seen_limits and val==env.etResourceLimits()[resource]:
        ...
        ## Regardless of where we are in an episode
        ...
        ## TODO: For now just call initphase and all the planning stuff at every step. Later add the plan-storing business
        ...
        ## TODO: To get the agent built we're bypassing this for now
        ...
        solution = random.choice([0, K_UP, K_DOWN, K_LEFT, K_RIGHT])
        ...
        #         movingTypes = self.checkForMovingTypes(env, self.hypotheses[0])
        #         if env.getTime()>compactStates[-1]['timestep']:
        #             scoreChange = env.getScore()!=compactStates[-1]['score']
        #         else:
        #             scoreChange = True
        #         # if self.display_text:
        #         # print "moving types: {}".format(movingTypes)
        #         # print "noNewObjectsInAWhile: {}".format(self.noNewObjectsInAWhile(env, self.noNewObjectNum))
        #         # print "scoreChange: {}".format(scoreChange)
        #         if self.noNewObjectsInAWhile(env, self.noNewObjectNum) and \
...
#                     return gameObject, win, score, steps, statesEncountered, effectsEncountered, compactStates, quit_level
        ...
        if not quitting:
            for i, action in enumerate(solution):
                ...
                for k, v in env._game.all_objects.iteritems():
                    self.memory.nextPositions[k] = (int(env._game.all_objects[k]['sprite'].rect.x), int(env._game.all_objects[k]['sprite'].rect.y))
                    try:
                ...
                self.max_game_time_observed = max(self.max_game_time_observed, env.getTime())
                if ended:
                    break
                ...
                if (i+1)%self.regrounding==0:
                    ...
                    if (not takingRandomSteps) and self.checkForDangerOrAvatarMisLocation(env, hypotheses[0], objectPositionsArray, i):
                        break
            ...
            print colored('________________________________________________________________', 'white', 'on_red')
            return gameObject, False, env.getScore(), steps, statesEncountered, effectsEncountered, compactStates, quit_level
