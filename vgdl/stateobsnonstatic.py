...
def buildTracker(rle):
	...
	for group in gameObject.sprite_groups.keys():
		...
		for sprite in gameObject.sprite_groups[group]:
			...
			trackedObjects[sprite.colorName].append(newSprite)
			if sprite.resources:
				try:
					for key in sprite.resources:
						try:
							...
							try:
								...
								color = rle._game.sprite_groups[key][0].colorName
						
						limit = rle._game.resources_limits[key]
						newSprite.inventory[color] = (sprite.resources[key], limit)
				except:
					print "in buildTracker"
					embed()
