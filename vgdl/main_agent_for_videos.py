...
from util import *
from core_for_videos import colorDict, VGDLParser, sys, keyPresses
...
import time
import copy
from agent_utils import translate_events, observe
...
class Agent:
    def __init__(self, modelType, gameFilename, hyperparameter_sets, hyperparameter_index=0, movieName=None):
    ...
    def fastcopy(self, rle):
        ...
        # newRle._game.lastcollisions = quickcopy(rle._game.lastcollisions)
    ...
    def makeMovie(self):
        VGDLParser.playGame(self.gameString, self.levelString, self.statesEncountered, \
            persist_movie=True, make_images=True, make_movie=True, movie_dir="tmp_videos/"+self.gameFilename, padding=10)

        print "Creating Movie"
        movie_dir = "tmp_videos/"+self.gameFilename
                ...
                for i, action in enumerate(solution):
                    ...
                    if self.reground_for_npcs: ## this is just exercising caution when near random objects, irrespective of whether they kill us or not
                        try:
                            ...
                            possiblePairList = [manhattan_distance(avatar, random)
    ...
    def executeStep(self, action, hypotheses, statesEncountered, run_induction=True):
        ...
        try:
            ...
            # agentState = quickcopy(self.rle._game.getAvatars()[0].resources)
        ...
        # effects = translate_events(res['effectList'], self.all_objects, self.rle)
