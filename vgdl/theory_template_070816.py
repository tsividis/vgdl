...
class Game(object):
	...
	def induction(self, trace):
		...
		for i in range(len(trace)): #iterate through each timestep
			...
			print ""
			for j in range(len(self.hypothesisSpace)): 			# Iterate through each theory
				theory = self.hypothesisSpace[i]
				if theory.likelihood(timestep) < 1.0: 	# Theory needs to be changed
					print "expanding on theory {}".format(j)
...
class InteractionRule(object):
	...
	'''
	def __init__(self, interaction, c1, c2):
