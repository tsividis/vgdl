...
echo ---------------- >> jobs.txt
echo --- $(date): Running fmri_makeMovie for subjects ${subj_arg} in parallel >> jobs.txt
echo ---------------- >> jobs.txt
head -n 1 gitlog.txt >> jobs.txt


for subj in ${subjects[*]}; do
    outfileprefix="${MY_SCRATCH}/VGDL/output/fmri_makeMovie_${subj}"
    echo ---------------------------------------------------------------------------------
    ...
    echo fmri_makeMovie.sh for subject ${subj}: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
