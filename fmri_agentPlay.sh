...
for subj in ${subjects[*]}; do
    for game in ${games[*]}; do
        ...
        sbatch_output=`sbatch -p fasse --mem 140000 -t 4-0:00 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="./run_fmri_agentPlay.sh ${agent} ${subj} ${game} ${steps_per_level} ${tag} ${metacontroller_index} ${epsilon_greedy} ${max_nodes_scale} ${insert} ${record_video_info}"`
        # for local testing
