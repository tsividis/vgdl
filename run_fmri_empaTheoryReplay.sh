# wrapper around fmri_empaTheoryReplay.py for executing inside jobs on the cluster
...
echo running empa theory replay for subj ${1}, game ${2}
...
do
    ...
    do
        ...
        do
            ...
            #out=`mongo --host holy7c22306.rc.fas.harvard.edu heroku_7lzprs54 --eval "db.plays.count({'subj_id': '${1}', 'run_id': ${run}, 'block_id': ${block}, 'instance_id': ${instance}, 'game_name': '${2}'})"` # ncf
            out=`mongo --host holy2a05206.rc.fas.harvard.edu heroku_7lzprs54 --eval "db.plays.count({'subj_id': '${1}', 'run_id': ${run}, 'block_id': ${block}, 'instance_id': ${instance}, 'game_name': '${2}'})"` # cannon
            ...
            do
                ...
                out=`mongo --host holy2a05206.rc.fas.harvard.edu heroku_7lzprs54 --eval "db.plans.count({'subj_id': '${1}', 'game_name': '${2}'})"`
                ...
                # run empaTheoryReplay
                echo ---- run_fmri_empaTheoryReplay: subj ${1}, run $run, block $block, instance $instance, play $play, game ${2}
                cmd="python -m cProfile -s cumtime fmri_empaTheoryReplay.py ${1} ${run} ${block} ${instance} ${play} ${2} ../../matlab/VGDL_fMRI/mat/decode_gp_CV_subj\=1_test.mat"
                echo ${cmd}
                eval ${cmd}
