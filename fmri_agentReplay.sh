...
for subj in ${subjects[*]}; do
    for game in ${games[*]}; do
        ...
        #
        sbatch_output=`sbatch -p fasse_gpu --gres=gpu --mem 20001 -t 0-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="./run_fmri_agentReplay.sh ${agent} ${subj} ${game}"`
        # for local testing
