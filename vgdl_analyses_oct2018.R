...
representation_lesion_games = c('aliens', 'butterflies', 'chase', 'corridor', 'helper', 'frogs', 'portals', 'sokoban', 'survivezombies', 'zelda')
data = subset(data, grepl('PL=', agent_type)&game_name%in%representation_lesion_games)

data = rbind(data, subset(MEPdata3, game_name%in%representation_lesion_games))
...
data = rbind(data, subset(humandata, game_name%in%representation_lesion_games))
...
p=ggplot(subset(plantimedata, game_name!='sokoban' & !(agent_type%in%c('human', 
                ...
                , aes(agent_type, game_name, fill=log(EMPA_normed_composite_ratio)))+geom_tile()+
