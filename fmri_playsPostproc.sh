# run fmri_playsPostproc for a bunch of subjects
...
for subj in ${subjects[*]}; do
        ...
        # send the job to NCF
        #
        sbatch_output=`sbatch -p shared --mem 20001 -t 1-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="source activate pedro; python fmri_playsPostproc.py ${subj}"`
        # for local testing
        ...
        echo fmri_playsPostproc.sh for subject ${subj}: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
