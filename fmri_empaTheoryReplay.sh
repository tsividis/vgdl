# run fmri_empaTheoryReplay for a bunch of subjects
...
games=( 'vgfmri3_lemmings'  )
#games=( 'vgfmri3_chase' 'vgfmri3_zelda')
#games=( 'none' )
...
for subj in ${subjects[*]}; do
    for game in ${games[*]}; do
        ...
        # send the job to NCF
        #
        sbatch_output=`sbatch -p ncf --mem 20001 -t 1-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="./run_fmri_empaTheoryReplay.sh ${subj} ${game}"`
        # for local testing
        ...
        echo fmri_empaTheoryReplay.sh for subject ${subj}, game ${game}: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
