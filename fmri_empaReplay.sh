...
#games=( 'vgfmri3_chase' 'vgfmri3_helper' 'vgfmri3_bait'  'vgfmri3_zelda' )
#games=( 'vgfmri3_chase')
games=( 'none' )
...
for subj in ${subjects[*]}; do
    for game in ${games[*]}; do
        outfileprefix="${MY_SCRATCH}/VGDL/output/fmri_empaReplay_${subj}_${game}"
        echo ---------------------------------------------------------------------------------
        echo Subject ${subj}, game ${game}, file prefix = $outfileprefix

        # send the job to NCF
        # mem 20000 for all but lemmings & plaqueattach; for them, 90000
        # OOM helper; up to 50000
        # sprites % 20: time 0-15 for all but lem & PA; for them, 1-15
        #
        sbatch_output=`sbatch -p shared --mem 20001 -t 2-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="./run_fmri_empaReplay.sh ${subj} ${game}"`
        # for local testing
