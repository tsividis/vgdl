...
#SBATCH --array=0-15
#SBATCH --output=slurm_logs/short_horizon_sorted_terminations/array_%A_%a.out
#SBATCH --time=960
...
if [ ! -d "${ROOT}/slurm_logs/short_horizon_sorted_terminations" ]; then
    mkdir "${ROOT}/slurm_logs/short_horizon_sorted_terminations"
fi
