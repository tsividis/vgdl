...
lst = []
for k,v in games_to_hyperparameters.mapping.items():
	x = itertools.repeat(k)
	lst.extend(zip(x,v))

def run_models(game_name, hyperparam_index):
	call_string = "python -m vgdl.load_games --game_name {} --hyperparameter_index {}".format(game_name, hyperparam_index)
	# time.sleep(10)
